...
function startWorkerRun({
  ...
  async function handleHostFunctionRequest(message, bridgeIndex) {
    ...
    try {
      ...
      if (outcome.status === "interrupted") {
        const interruptionId = replayEntry?.status === "interrupted" ? replayEntry.interruptionId : `interrupt-${bridgeIndex}`;
  ...
  function settleInterruptionsIfQuiescent() {
    if (pendingInterruptionIndexes.size > 0 && inFlightBridgeRequests === 0 && workerIdleRequestCount >= totalBridgeRequests) {
      runInBackground(settleInterruptions());
    }
  }
  function assertReplayMatches(entry, message) {
    if (entry.bindingName !== message.hostFunctionName || entry.inputJson !== message.inputJson) {
      throw new RunProtocolError(
